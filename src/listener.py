...
#   ----------  --------------- ------------------------------------
#   2017-09-22  reporting-tech  Created
# *********************************************************************
...
from rtech.listener.listener_factory import ListenerFactory
...
def init_args():
    ...
    parser.add_argument('--environment', default='lab', required=True, choices=['lab', 'prod'])
    parser.add_argument('--type', required=True, choices=['amqp'])
    parser.add_argument('--name', required=True, choices=['rabbitmq'])
...
if __name__ == "__main__":
    ...
    logger.info(' * Parameter: listener_type = ' + args.type)
    logger.info(' * Parameter: listener_name = ' + args.name)
    logger.info('')

    cc = ConfigContainer(args.environment, '', '', config)
    li = ListenerFactory(cc).new_listener(args.type, args.name, None)
    li.listen()
